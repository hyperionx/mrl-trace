...
It does so with a deliberately minimal, provenance-labelled model:
  ...
  STATE 1 (the TAG, volatile)  -- the eligibility trace ``v(t)``.
      MODEL-GROUNDED. Generated by the approximate
      :class:`mrl_trace.device.CascadeEligibilityGate`
      (trap-cascade rise plus a caller-labelled ``tau_leak`` leakage). The shape is a
      computational surrogate; neither the chosen retention nor its transfer from ITO
      to this hypothetical capture device is established by this module.
...
The exploratory observable is the **capture curve**: the committed weight change as a
...
``tau_leak``. This behaviour follows from the assumed coupling and is not a measured
physical signature of the present two-terminal cell.
"""
...
from .device import CascadeEligibilityGate
...
__all__ = [
    ...
    "TwoStateSynapse",
    "capture_curve",
    "CAPTURE_METHOD_PROVENANCE",
]

CAPTURE_METHOD_PROVENANCE = {
    "status": "extrapolated",
    "established_basis": ["volatile-tag/non-volatile-capture device concepts"],
    "repository_adaptation": (
        "An assumed Hill coupling maps the cascade-surrogate tag to one step on a "
        "measured conductance ladder."
    ),
    "claim_limit": (
        "The coupling is unmeasured in this SiOx cell, so this is future-work model "
        "exploration and not evidence for device-native capture."
    ),
}
...
class TwoStateSynapse:
    ...
    The tag ``v(t)`` is the cascade eligibility surrogate
    (:class:`CascadeEligibilityGate`).  The capture coupling remains assumed.
    ...
    def __init__(self, V: float = 0.9, tau_leak: float = 2.0, dt: float = 0.05,
        ...
        self.gate = CascadeEligibilityGate(V=V, tau_leak=tau_leak, dt=dt)
    ...
    def run(self, t_grid: np.ndarray, coincidence_at: float, reward_at: float,
        ...
        return {"t": t_grid, "v": v_ts, "G": G_ts, "dG": self.G - G0, "G0": G0,
                ...
                "method_provenance": CAPTURE_METHOD_PROVENANCE}
